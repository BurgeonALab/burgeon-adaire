...
export default class BSmallValbury extends Component {
  ...
  render() {
    ...
    var valorderDate = moment(new Date(this.state.xauusd.date))
      .locale("id")
      .format("ll");
    ...
    function DataTradeClean($) {
      ...
      $(".ordering-badges-valbury-mobile").attr(
        "style",
    ...
    function DataTradeUnavailable($) {
      ...
      if (
        dateday === "Sabtu" ||
        dateday === "Minggu" ||
        (dateday === "Senin" && datetime <= timeopen)
      ) {
          ...
          <div className="valbury-box-container-mobile h-100 d-flex flex-column justify-content-between">
            ...
            <div className="d-flex flex-row vaf-slide-container">
              <a
                role="button"
                onClick={goldSlider}
                className="vaf-slide-button gold active"
              ></a>
              <a
                role="button"
                onClick={crudeSlider}
                className="vaf-slide-button crude"
              ></a>
              <a
                role="button"
                onClick={yenSlider}
                className="vaf-slide-button yen"
              ></a>
            </div>
            <div className="d-flex flex-column justify-content-center">
              <div className="signal-order-box-custom">
                ...
                <p
                  id="signal-order"
                  className="signal-order-text text-light text-end fw-bold mb-0"
                >
                  {this.state.xauusd.order}
                </p>
                <div className="d-flex flex-row data-detail-valbury">
                  <div className="flex-grow-1">
                    <p className="signal-order-description mb-0 text-end fw-bold my-1 text-light">
                      Price:
                    </p>
                    <p className="signal-order-description mb-0 text-end fw-bold my-1 text-light">
                      Stop Loss:
                  ...
                  <div>
                    <p
                      id="signal-price"
                      className="signal-order-description mb-0 text-end fw-medium my-1 text-light"
                    >
                      &nbsp;{this.state.xauusd.price}
                    </p>
                    <p
                      id="signal-sl"
                      className="signal-order-description mb-0 text-end fw-medium my-1 text-light"
                    >
                      &nbsp;{this.state.xauusd.stoploss}
                    </p>
                    <p
                      id="signal-tp1"
                      className="signal-order-description mb-0 text-end fw-medium my-1 text-light"
                    >
                      &nbsp;{this.state.xauusd.takeprofit1}
                    </p>
                    <p
                      id="signal-tp2"
                      className="signal-order-description mb-0 text-end fw-medium my-1 text-light"
                    >
                      &nbsp;{this.state.xauusd.takeprofit2}
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="ordering-badges-valbury-mobile d-flex flex-column align-items-end">
              <span
                id="status-market"
                className="badge text-bg-secondary badge-fit-content mt-2"
              >
                Status Market
              </span>
              <span
